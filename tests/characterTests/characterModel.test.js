...
//#endregion
//#region Add Item, spell and remove
...
test('levelUp - Success', async () =>
{
    ...
    expect(Array.isArray(db2)).toBe(true);
    expect(db2.length).toBe(1);
    expect(db2[0].Level).toBe(randomCharacter.Level + 1);
});
test('levelUp - Fail', async () =>
{
    // Add 3 users since the highest user id in a random character is 3
    await userModel.addUser('user1', 'Password1');
    await userModel.addUser('user2', 'Password2');
    ...
    await expect(characterModel.levelUp(id + 1)).rejects.toThrow(InvalidInputError);
    await expect(characterModel.levelUp(id + 1)).rejects.toThrow(`Character does not exist`);
});
test('updateExp - Success', async () =>
{
    // Add 3 users since the highest user id in a random character is 3
    await userModel.addUser('user1', 'Password1');
    await userModel.addUser('user2', 'Password2');
    ...
    await characterModel.updateExp(id, randomCharacter.Experience);

    let db2 = await characterModel.getUserCharacters(randomCharacter.UserId);
    expect(Array.isArray(db2)).toBe(true);
    expect(db2.length).toBe(1);
    expect(db2[0].Experience).toBe(randomCharacter.Experience);
});
test('updateExp - Fail', async () =>
{
    // Add 3 users since the highest user id in a random character is 3
    await userModel.addUser('user1', 'Password1');
    await userModel.addUser('user2', 'Password2');
    ...
    await expect(characterModel.updateExp(id + 1, randomCharacter.Experience)).rejects.toThrow(InvalidInputError);
    await expect(characterModel.updateExp(id + 1, randomCharacter.Experience)).rejects.toThrow(`Character does not exist`);
});
test('updateAC - Success', async () =>
{
    // Add 3 users since the highest user id in a random character is 3
    await userModel.addUser('user1', 'Password1');
    await userModel.addUser('user2', 'Password2');
    ...
    expect(Array.isArray(db)).toBe(true);
    expect(db.length).toBe(1);
    expect(db[0].ArmorClass).toBe(randomCharacter.ArmorClass);


    const randomCharacter2 = getRandomCharacterSplice();
    await characterModel.updateAC(id, randomCharacter2.ArmorClass);

    let db2 = await characterModel.getUserCharacters(randomCharacter.UserId);
    expect(Array.isArray(db2)).toBe(true);
    expect(db2.length).toBe(1);
    expect(db2[0].ArmorClass).toBe(randomCharacter2.ArmorClass);
});
test('updateAC - Fail', async () =>
{
    // Add 3 users since the highest user id in a random character is 3
    await userModel.addUser('user1', 'Password1');
    await userModel.addUser('user2', 'Password2');
    ...
    const randomCharacter2 = getRandomCharacterSplice();
    await expect(characterModel.updateAC(id + 1, randomCharacter2.ArmorClass)).rejects.toThrow(InvalidInputError);
    await expect(characterModel.updateAC(id + 1, randomCharacter2.ArmorClass)).rejects.toThrow(`Character does not exist`);
});
test('updateSpeed - Success', async () =>
{
    // Add 3 users since the highest user id in a random character is 3
    await userModel.addUser('user1', 'Password1');
    await userModel.addUser('user2', 'Password2');
    ...
    expect(Array.isArray(db)).toBe(true);
    expect(db.length).toBe(1);
    expect(db[0].Speed).toBe(null);


    const randomCharacter2 = getRandomCharacterSplice();
    await characterModel.updateSpeed(id, randomCharacter2.Speed);

    let db2 = await characterModel.getUserCharacters(randomCharacter.UserId);
    expect(Array.isArray(db2)).toBe(true);
    expect(db2.length).toBe(1);
    expect(db2[0].Speed).toBe(randomCharacter2.Speed);
});
test('updateSpeed - Fail', async () =>
{
    // Add 3 users since the highest user id in a random character is 3
    await userModel.addUser('user1', 'Password1');
    await userModel.addUser('user2', 'Password2');
    ...
    const randomCharacter2 = getRandomCharacterSplice();
    await expect(characterModel.updateSpeed(id + 1, randomCharacter2.Speed)).rejects.toThrow(InvalidInputError);
    await expect(characterModel.updateSpeed(id + 1, randomCharacter2.Speed)).rejects.toThrow(`Character does not exist`);
});
test('updateInitiative - Success', async () =>
{
    // Add 3 users since the highest user id in a random character is 3
    await userModel.addUser('user1', 'Password1');
    await userModel.addUser('user2', 'Password2');
    ...
    expect(Array.isArray(db)).toBe(true);
    expect(db.length).toBe(1);
    expect(db[0].Initiative).toBe(null);


    const randomCharacter2 = getRandomCharacterSplice();
    await characterModel.updateInitiative(id, randomCharacter2.Initiative);

    let db2 = await characterModel.getUserCharacters(randomCharacter.UserId);
    expect(Array.isArray(db2)).toBe(true);
    expect(db2.length).toBe(1);
    expect(db2[0].Initiative).toBe(randomCharacter2.Initiative);
});
test('updateInitiative - Fail', async () =>
{
    // Add 3 users since the highest user id in a random character is 3
    await userModel.addUser('user1', 'Password1');
    await userModel.addUser('user2', 'Password2');
    ...
    const randomCharacter2 = getRandomCharacterSplice();
    await expect(characterModel.updateInitiative(id + 1, randomCharacter2.Initiative)).rejects.toThrow(InvalidInputError);
    await expect(characterModel.updateInitiative(id + 1, randomCharacter2.Initiative)).rejects.toThrow(`Character does not exist`);
});
//#endregion
//#region COOKIE
test('createRecentCharactersCookie - Success New Cookie', async () =>
{
    // Add 3 users since the highest user id in a random character is 3
    await userModel.addUser('user1', 'Password1');
    await userModel.addUser('user2', 'Password2');
    ...
    const randomCharacter = getRandomCharacterSplice();
    let id = await characterModel.addCharacterObject(randomCharacter);
    let cookie = await characterModel.createRecentCharactersCookie(id);
    expect(Array.isArray(cookie.recentCharacters)).toBe(true);
    expect(cookie.recentCharacters.length).toBe(1);

    expect(cookie).toHaveProperty('name');
    expect(cookie).toHaveProperty('expires');
    expect(cookie.recentCharacters[0]).toHaveProperty('id');

    const randomCharacter2 = getRandomCharacterSplice();
    //Make sure userId is the same
    randomCharacter2.UserId = randomCharacter.UserId;
    let id2 = await characterModel.addCharacterObject(randomCharacter2);
    let cookie2 = await characterModel.createRecentCharactersCookie(id2, cookie.recentCharacters);
    expect(Array.isArray(cookie.recentCharacters)).toBe(true);
    expect(cookie.recentCharacters.length).toBe(2);

    expect(cookie2).toHaveProperty('name');
    expect(cookie2).toHaveProperty('expires');
    expect(cookie2.recentCharacters[0]).toHaveProperty('id');
    expect(cookie2.recentCharacters[0].id).toBe(id2);
    expect(cookie2.recentCharacters[1].id).toBe(id);
});
test('createRecentCharactersCookie - Fail', async () =>
{
    // Add 3 users since the highest user id in a random character is 3
    await userModel.addUser('user1', 'Password1');
    await userModel.addUser('user2', 'Password2');
    ...
    await expect(characterModel.createRecentCharactersCookie(id2, cookie)).rejects.toThrow(InvalidInputError);
    await expect(characterModel.createRecentCharactersCookie(id2, cookie)).rejects.toThrow(`previousRecents must be an array of integers`);
});
//#endregion
